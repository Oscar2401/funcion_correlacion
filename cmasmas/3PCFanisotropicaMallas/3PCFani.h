...
class NODE3P{
	...
	public:
		...
		void BPC_xxy(float *****, PointW3D *, PointW3D *);
		~NODE3P();
};
...
void NODE3P::make_histoXXY(float *****XXY, Node ***nodeX, Node ***nodeY, PointW3D *datx, PointW3D *daty){
	...
	*/ 
	...
	std::cout << "-> Estoy haciendo histograma XXY..." << std::endl;
	// x1N, y1N, z1N => Nodo pivote
	for (row=0; row<partitions; ++row){
	x1N = nodeX[row][0][0].nodepos.x;
	for (col=0; col<partitions; ++col){
...
//=================================================================== 
void NODE3P::BPC_xxy(float *****XXY, PointW3D *data, PointW3D *rand){
